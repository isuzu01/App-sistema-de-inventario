...
object FirebaseProveedorRepository {
    ...
    suspend fun insetarProveedorFirebaseYRoom(proveedorDao: ProveedorDao, proveedor: ProveedorEntity){
    ...
    suspend fun actualizarProveedorFirabaseYRoom(proveedorDao: ProveedorDao, proveedor: ProveedorEntity){
        withContext(Dispatchers.IO){
            try {
    ...
    suspend fun eliminarProveedorFirebaseYRoom(proveedorDao: ProveedorDao, proveedor: ProveedorEntity){
        withContext(Dispatchers.IO){
            try {
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    try {
                        ...
                        for (firebaseProveedores in proveedoresFirebase){
                            try {
                                val proveedorExistente = proveedorDao.getProveedorById(firebaseProveedores.id)
                                ...
                                Log.e(TAG, "Error sincronizando producto ID ${firebaseProveedores.id}: ${e.message}")
                            }
                        }
                        Log.d( TAG,"Sincronizacion completa. Proveedores: ${proveedoresFirebase.size}")
                    }catch (e: Exception){
